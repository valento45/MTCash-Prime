...
namespace MTBE_u.EntitiesCash
{
    public class Receita : Conta
    {
        ...
        {
            SqlCommand cmd = new SqlCommand("insert into mtcash.u_tb_receita (descricao, data_inicio, data_recibo, pagante, valor_receita, desconto) values (" +
                $"'{Descricao}', '{Data_Inicio}', '{Data_Vecimento}', '{Valor}', '{Desconto}')");
            Access.ExecuteNonQuery(cmd);
        }

        public override void Update(Conta conta)
        {
            SqlCommand cmd = new SqlCommand("update mtcash.u_tb_receita set descricao = @descricao, data_inicio = @data_inicio, data_recibo = @data_recibo, pagante = @pagante, valor_receita = @valor_receita, desconto = @desconto where id_receita = @id_receita");
